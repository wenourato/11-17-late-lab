...
// CODE HERE
const getEvenNums  = mixedNumbers.filter(evenNums => evenNums % 2 ===0)
...
const mappedPrice = prices.map(price => price * 1.07)
...
const ordersAfterTax = orders.map(function(elem) {
  return elem.price + (elem.price * elem.tax)
})

const ordersAfterTax = orders.map(ordrs => orders.price + (order.price * order.tax))

// still having problems figureing out what exactly to console log


// const multiply = (price, tax) => {
//   return num1 * num2
// }

// const taxCalc = (price, tax, callback) => {
//   if (+num1 && +num2){
//     price = +tax
//     tax = +tax
...
//   }
// }
// console.log(taxCalc(price, tax, multiply))
